...
void startSystem::startOS() {
    ...
    while (true) {
        ...
        getline(cin, input);
        if (input.empty()) {
            cerr << "provide good input" << endl;
        } else {
            checkInput(input, readyQueue);
            if (readyQueue.empty()) {} else {
                preemptiveCheck();
            }
        }
    }
    ...
    if (inputStr.at(0) == 'A') {
        ...
        if (inputStr.size() != 1) {
            ...
            if (readyQ.empty()) {
                ...
                cpu.terminateP();
                cerr << "No process to terminate from ready queue, only cpu" << endl;
            } else {
                removeProcessInMemory(cpu.getProcess().get_ValID());
                processTable.erase(cpu.getProcess().get_ValID());
        ...
        if (result.at(1) == "r") {
            ...
            for (int pos = 0; pos < ram.size(); pos++) {
                cout << "ID: " << ram.at(pos).PID << "    Begin Mem: " << ram.at(pos).beginMem << "    End Mem: " << ram.at(pos).endMem << endl;
            }
        }
    } else if (inputStr.at(0) == 'T') {
